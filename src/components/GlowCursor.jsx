...
const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    let raf = 0;
    let destroyed = false;
    const isGlobal = !!initialConfig.global;
    ...
    const moveTarget = isGlobal ? window : container;
    const enterTarget = isGlobal ? window : container;

    const handleMove = (e) => {
      // For global we always consider inside
      if (isGlobal) pointerInside = true;
      updatePointer(e);
    };

    moveTarget.addEventListener('pointermove', handleMove);
    enterTarget.addEventListener('pointerenter', updatePointer);
    if (!isGlobal) {
      container.addEventListener('pointerleave', onPointerLeave);
    }

    // For global, also track when mouse leaves the whole document
    ...
    return () => {
      ...
      enterTarget.removeEventListener('pointerenter', updatePointer);
      if (!isGlobal) {
        container.removeEventListener('pointerleave', onPointerLeave);
      }
      if (isGlobal) {
        document.removeEventListener('pointerleave', handleDocLeave);
      }
  ...
  const containerStyle = global
    ? { position: 'fixed', inset: 0, zIndex: 99999, pointerEvents: 'none', ...style }
    : style;

  return (
    <div ref={containerRef} className={`glow-cursor${className ? ` ${className}` : ''}`} style={containerStyle} {...rest}>
