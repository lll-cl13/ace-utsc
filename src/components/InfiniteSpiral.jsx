...
const InfiniteSpiral = ({
  ...
  useEffect(() => {
    ...
    const render = time => {
      ...
      // Make card size flexible: grow/shrink with container width
      const sizeScale = clamp(width / 960, 0.55, 2.1);
      const respCardW = Math.round(cardWidth * sizeScale);
      const respCardH = Math.round(cardHeight * sizeScale);
      const respVSpacing = verticalSpacing * sizeScale;
      const respCardR = Math.round(cardRadius * Math.min(sizeScale, 1.7));

      const fit = Math.min(1.2, width / Math.max(respCardW * 2.6, 40), height / Math.max(respCardH * 2.2, 30));
      const responsiveRadius = Math.min(radius * sizeScale, Math.max(72, width * 0.36)) * fit;
      const fadeStart = clamp(1 - edgeFade, 0, 0.98);
      const turnSize = Math.max(cardsPerTurn, 1);

      // update CSS vars for initial card styles
      const rootEl = rootRef.current;
      if (rootEl) {
        rootEl.style.setProperty('--infinite-spiral-card-width', `${respCardW}px`);
        rootEl.style.setProperty('--infinite-spiral-card-height', `${respCardH}px`);
        rootEl.style.setProperty('--infinite-spiral-card-radius', `${respCardR}px`);
      }

      cardRefs.current.forEach((card, index) => {
        if (!card) return;
        const img = card.querySelector('img');
        card.style.width = `${respCardW}px`;
        card.style.height = `${respCardH}px`;
        card.style.borderRadius = `${respCardR}px`;
        if (img) {
          img.style.width = `${respCardW}px`;
          img.style.height = `${respCardH}px`;
        ...
        const y = offset * respVSpacing * fit;
        ...
        // Always add blur at top and bottom of the visible image area
        const cardCenterY = height * 0.5 + y;
        const distTop = Math.max(0, cardCenterY);
        const distBottom = Math.max(0, height - cardCenterY);
        const vZone = Math.max(50, height * 0.16);
        const vFactor = Math.max(
          1 - clamp(distTop / vZone, 0, 1),
          1 - clamp(distBottom / vZone, 0, 1)
        );
        const vBlur = edgeBlur * smoothstep(0.05, 0.95, vFactor);
        const totalBlur = Math.max(blur, vBlur);

        card.style.transform = `translate(-50%, -50%) translate3d(${x}px, ${y}px, 0) rotateZ(${cardTilt}deg) scale(${visualScale})`;
        card.style.opacity = opacity.toFixed(3);
        card.style.filter = totalBlur > 0.01 ? `blur(${totalBlur.toFixed(2)}px)` : 'none';
  ...
  const rootStyle = {
    ...
    '--infinite-spiral-fade-y': edgeFade > 0 ? `${(clamp(edgeFade, 0.02, 0.4) * 42).toFixed(1)}%` : '0%',
    '--infinite-spiral-fade-x': edgeFade > 0 ? `${(clamp(edgeFade, 0.02, 0.4) * 35).toFixed(1)}%` : '0%',
